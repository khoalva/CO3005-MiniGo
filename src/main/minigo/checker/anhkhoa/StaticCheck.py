...
class StaticChecker(BaseVisitor,Utils):
    ...
    def _compare_array_types(self, target: ArrayType, source: ArrayType, allowed_casts: List[Tuple[Type, Type]], env, allow_cast) -> bool:
        ...
        return self.is_compatible_type(target.eleType, source.eleType, allowed_casts, env, allow_cast)
    ...
    def visitAssign(self, ast, c):
        ...
        if isinstance(ast.lhs, Id):
            ...
            if not is_extended_op:  # Toán tử :=
                if lhs_symbol is None:  # Chưa khai báo -> khai báo mới
    ...
    def visitForEach(self, ast: ForEach, c: List[List[Symbol]]):
        ...
        # traverse_env(c, "visitForEach")  # Giả định đây là hàm debug trong hệ thống của bạn
    ...
    def visitReturn(self, ast, c):
        ...
        if ast.expr is not None:
            self.is_statement_context = False
            if not self.is_compatible_type(self.visit(ast.expr, c), self.current_function.rettype, [(FloatType, IntType)], c, False):
                raise TypeMismatch(ast)
            self.is_statement_context = True
        else:
    ...
    def visitFuncCall(self, ast, c):
        ...
        for arg, param in zip(ast.args, func_sym.mtype.partype):
            ...
            if not self.is_compatible_type(arg_type, param_type, [], c):
                raise TypeMismatch(ast)
    ...
    def visitId(self,ast,c):
        ...
        if res is None or isinstance(res.mtype, MType):
            raise Undeclared(Identifier(), ast.name)
        return res.mtype
